...
def submit(rec_methods, subject_idx,
    ...
    mem_gb = 16
    args = f"{rec_methods} {subject_idx}"
    ...
    execute_on_cluster(str(cluster_script_path),
                       ...
                       concurrency_tag = 'rendered_mead_comparison',
                       )   
    # t.sleep(1)
...
def main():
    ...
    rec_methods = ['emoca', 'spectre', 'EMOCA_v2_lr_mse_15_with_bfmtex', 'EMICA_v0_mp_lr_mse_15', 'EMICA_mead_mp_lr_mse_15']

    subjects = list(range(0, 47))
    # subjects = [18, 20, 29, 32, 33]
    # for i in range(0, len(rec_methods)):
        # subjects = list(range(1, 47))

        # for rec_method in rec_methods:
    for subject_idx in subjects:
        submit(','.join(rec_methods), subject_idx, bid=1500)
        # submit(','.join(rec_methods[:i]), subject_idx, bid=1500)
