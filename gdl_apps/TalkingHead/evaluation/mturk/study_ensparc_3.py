...
def design_study_3(model_a, model_b, num_videos, output_folder):
    ...
    for i in range(num_catch_trials):
        ...
        if not flip:
            hit_list_line = f"{video_a}#{video_b_lip}\n"
        else:
            hit_list_line = f"{video_b_lip}#{video_a}\n"
...
def main():
    ...
    for model_b in sota_models: 
        ...
        output_folder.mkdir(parents=True, exist_ok=True)
        design_study_3(main_model, model_b, 0, output_folder)
