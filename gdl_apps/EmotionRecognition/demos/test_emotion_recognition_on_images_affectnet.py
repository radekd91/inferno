...
def save_images(batch, predictions, output_folder):
    ...
    for i in range(len(batch["image"])):
        ...
        if expr == AffectNetExpressions.Contempt:
            plt.figure()
            ...
            out_fname = Path(output_folder) / f"{batch['image_name'][i]}_out.png"
            ...
            plt.close()
            imsave(Path(output_folder) / f"{batch['image_name'][i]}_in.png", img)
            print(f"Saving {expr.name}")
        else: 
            print(f"Skipping {expr.name}")


def main():
    parser = argparse.ArgumentParser()
    # add the input folder arg 
    parser.add_argument('--input_folder', type=str, default="/ps/project/EmotionalFacialAnimation/data/affectnet/Automatically_Annotated/Automatically_Annotated_Images/6")
    parser.add_argument('--output_folder', type=str, default="/ps/scratch/rdanecek/EMOCA/Contempt/emoca/6", help="Output folder to save the results to.")
