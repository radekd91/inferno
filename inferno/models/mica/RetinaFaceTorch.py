####################################################################################################
## The following code is copied over from the original insightface.model_zoo.retinaface.py file 
...
class RetinaFace:
    def __init__(self, model_path=None, model=None):
        ...
        if model is None:
            # onnx_model = onnx2torch.ImportModel(model_path)
            # self.model = onnx_model.convert_to_pytorch()
            #             onnx_model = onnx2torch.ImportModel(model_path)
    ...
    # ... (rest of the RetinaFace class implementation remains the same)
    ...
    def _init_vars(self):
        ...
        if len_outputs==6:
            ...
            raise NotImplementedError('Invalid output number: {}'.format(len_outputs))
    ...
    def forward(self, img, threshold):
        ...
        ## the new pytorch-based way
        blob = torch.from_numpy(blob)
        with torch.no_grad():
            net_outs = self.model(blob)
        net_outs = [o.numpy() for o in net_outs]
